...
app.get('/auth', (req, res) => {
    const authUrl = `https://www.instagram.com/oauth/authorize?client_id=${CLIENT_ID}&redirect_uri=${REDIRECT_URI}&scope=user_profile,user_media&response_type=code`;
...
app.get('/auth/callback', async (req, res) => {
    ...
    try {
        ...
        const response = await axios.post('https://www.instagram.com/oauth/access_token', {
            client_id: APP_ID,
...
app.use(express.static(path.join(__dirname, "../../messagingClient/src")));
...
app.get("/logout", function (req, res) {
    if (req.session) {
        req.session.destroy((err) => {
            if (err) {
                res.render("login", { error: "Unable to log out" });
            } else {
                res.render("login", { response: "Logout successful!" });
            }
        });
    } else {
        res.render("login", { response: "Token not stored in session" });
    }
});
/*
https
...
client.picker({
  onUploadDone: (res) => {
    console.log('File uploaded successfully:', res.filesUploaded[0].url);
  }
}).open();
